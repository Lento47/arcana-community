...
const TIPS: Tip[] = [
  ...
  "Create {highlight}arcana.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/arcana/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.arcana/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.arcana/agents/{/highlight} for specialized AI personas",
  ...
  "Use {highlight}arcana run{/highlight} for non-interactive scripting",
  "Use {highlight}arcana --continue{/highlight} to resume the last session",
  "Use {highlight}arcana run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}arcana serve{/highlight} for headless API access to arcana",
  "Use {highlight}arcana run --attach{/highlight} to connect to a running server",
  ...
  "Create JSON theme files in {highlight}.arcana/themes/{/highlight} directory",
  ...
  "Run {highlight}arcana debug config{/highlight} to troubleshoot configuration",
  ...
  "Type /goal to set a session goal — the agent tracks it on a kanban board",
  "Type /loop to check in on goal progress — the agent reports what's done and pending",
