import type { Event } from "@arcana/sdk/v2"
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@arcana/plugin/tui"
...
const tui: TuiPlugin = async (api) => {
  ...
  api.event.on("session.status", (event) => {
    ...
    notify(api, sessionID, COPY.riteComplete, session?.parentID ? "subagent_done" : "done")
  })
