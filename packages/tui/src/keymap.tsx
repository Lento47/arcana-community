...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ARCANA_MODE_KEY, ARCANA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ARCANA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ARCANA_MODE_KEY, stack.at(-1)?.mode ?? ARCANA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ARCANA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ARCANA_MODE_KEY, undefined)
