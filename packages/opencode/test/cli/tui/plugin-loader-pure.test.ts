...
`,
  ...
  const pure = process.env.ARCANA_PURE
  const meta = process.env.ARCANA_PLUGIN_META_FILE
  process.env.ARCANA_PURE = "1"
  process.env.ARCANA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.ARCANA_PURE
    } else {
      process.env.ARCANA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.ARCANA_PLUGIN_META_FILE
    } else {
      process.env.ARCANA_PLUGIN_META_FILE = meta
    }
  }
})
