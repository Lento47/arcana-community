...
//   - ARCANA_CONFIG_CONTENT      : provider config inline, no files to find
//   - ARCANA_TEST_HOME           : pins os.homedir() → tmpdir
//   - ARCANA_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - ARCANA_PURE                : skip external plugin discovery + install
//   - ARCANA_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    ARCANA_CONFIG_CONTENT: configJson,
    ARCANA_DISABLE_PROJECT_CONFIG: "1",
    ARCANA_PURE: "1",
    ARCANA_DISABLE_AUTOUPDATE: "1",
    ARCANA_DISABLE_AUTOCOMPACT: "1",
    ARCANA_DISABLE_MODELS_FETCH: "1",
    ARCANA_AUTH_CONTENT: "{}",
  }
}
...
function expectExit(result: RunResult, expected: number, label = "arcana") {
