...
import { LLMEvent, ToolFailure } from "@arcana/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@arcana/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("arcana"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("arcana") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("arcana") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
