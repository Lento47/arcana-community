...
import { EventSequenceTable } from "@arcana/core/event/sql"
...
import { WorkspaceTable } from "@arcana/core/control-plane/workspace.sql"
...
const originalEnv = {
  ARCANA_AUTH_CONTENT: process.env.ARCANA_AUTH_CONTENT,
  ARCANA_EXPERIMENTAL_WORKSPACES: process.env.ARCANA_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.ARCANA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.ARCANA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.ARCANA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.ARCANA_EXPERIMENTAL_WORKSPACES).toBe("true")
