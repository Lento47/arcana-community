  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("ARCANA_SERVER_PASSWORD", original.envPassword)
        restoreEnv("ARCANA_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
...
describe("HttpApi UI fallback", () => {
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "arcana" }).request("/", {
        method: "OPTIONS",
        headers: {
