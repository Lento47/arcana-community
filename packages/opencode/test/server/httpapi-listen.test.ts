...
const original = {
  ...
  envPassword: process.env.ARCANA_SERVER_PASSWORD,
  envUsername: process.env.ARCANA_SERVER_USERNAME,
}
const auth = { username: "arcana", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.ARCANA_SERVER_PASSWORD = original.ARCANA_SERVER_PASSWORD
  Flag.ARCANA_SERVER_USERNAME = original.ARCANA_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.ARCANA_SERVER_PASSWORD
  else process.env.ARCANA_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.ARCANA_SERVER_USERNAME
  else process.env.ARCANA_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.ARCANA_SERVER_PASSWORD = auth.password
  Flag.ARCANA_SERVER_USERNAME = auth.username
  process.env.ARCANA_SERVER_PASSWORD = auth.password
  process.env.ARCANA_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.ARCANA_SERVER_PASSWORD = undefined
  Flag.ARCANA_SERVER_USERNAME = auth.username
  delete process.env.ARCANA_SERVER_PASSWORD
  process.env.ARCANA_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
