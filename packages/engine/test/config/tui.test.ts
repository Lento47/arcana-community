...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.ARCANA_CONFIG
  delete process.env.ARCANA_TUI_CONFIG
...
it.instance("project config takes precedence over ARCANA_TUI_CONFIG (matches ARCANA_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("ARCANA_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from ARCANA_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
