...
import { EffectFlock } from "@arcana/core/util/effect-flock"
...
import { ConfigPluginV1 } from "@arcana/core/v1/config/plugin"
...
const managedConfigDir = process.env.ARCANA_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.ARCANA_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.ARCANA_CONSOLE_TOKEN
  else process.env.ARCANA_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
...
it.effect("does not create global config when ARCANA_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["arcana"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.effect("does not try to install dependencies in read-only ARCANA_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("ARCANA_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable ARCANA_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
// Note: preload.ts sets ARCANA_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("ARCANA_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { ARCANA_CONFIG_DIR: undefined, ARCANA_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "ARCANA_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { ARCANA_DISABLE_PROJECT_CONFIG: "true", ARCANA_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed ARCANA_PERMISSION JSON used to crash
...
describe("ARCANA_PERMISSION env var", () => {
  it.instance("does not crash when ARCANA_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "ARCANA_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("ARCANA_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in ARCANA_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in ARCANA_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
