...
import { Hash } from "@arcana/core/util/hash"
...
import { ProjectDirectories } from "@arcana/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const opencodeFile = path.join(tmp, ".git", "arcana")
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "arcana")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "arcana")
      const wrongCache = path.join(parentDir, ".git", "arcana")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "arcana")
      const cacheB = path.join(bareB, "arcana")
      const wrongCache = path.join(parentDir, ".git", "arcana")
