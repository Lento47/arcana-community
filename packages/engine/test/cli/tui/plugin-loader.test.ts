...
import type { TuiAttentionSoundPack } from "@arcana/plugin/tui"
...
const { allThemes, addTheme } = await import("@arcana/tui/context/theme")
...
`,
  ...
  process.env.ARCANA_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.ARCANA_PLUGIN_META_FILE!,
    )
