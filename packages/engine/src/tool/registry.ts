...
export class Service extends Context.Service<Service, Interface>()("@arcana/ToolRegistry") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    let _toolCache: { tools: Tool.Def[]; time: number; key: string } | null = null

    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const cacheKey = `${input.providerID}:${input.modelID}:${input.agent.name}`
      if (_toolCache && _toolCache.key === cacheKey && Date.now() - _toolCache.time < 60000) {
        return _toolCache.tools
      }
