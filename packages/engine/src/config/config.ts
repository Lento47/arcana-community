...
import { RemoteAuthError } from "@arcana/core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@arcana/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["opencode.jsonc", "opencode.json", "arcana.jsonc", "arcana.json", "config.json"].map((file) =>
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ARCANA_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://arcana.ai/config.json"
        ...
        if (Flag.ARCANA_CONFIG) {
          yield* merge(Flag.ARCANA_CONFIG, yield* loadFile(Flag.ARCANA_CONFIG, authEnv))
        ...
        if (Flag.ARCANA_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ARCANA_CONFIG_DIR", { path: Flag.ARCANA_CONFIG_DIR })
        }
        ...
        if (process.env.ARCANA_CONFIG_CONTENT) {
          const source = "ARCANA_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ARCANA_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ARCANA_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ARCANA_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ARCANA_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ARCANA_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ARCANA_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("ARCANA_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.ARCANA_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ARCANA_DISABLE_PRUNE) {
