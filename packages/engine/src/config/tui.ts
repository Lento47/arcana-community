...
import { isRecord } from "@arcana/tui/util/record"
...
import { TuiKeybind } from "@arcana/tui/config/keybind"
...
import { TuiConfig } from "@arcana/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@arcana/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and ARCANA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ARCANA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ARCANA_TUI_CONFIG override, if set.
  if (Flag.ARCANA_TUI_CONFIG) {
    const configFile = Flag.ARCANA_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and ARCANA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir.endsWith(".arcana") || dir === Flag.ARCANA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && !dir.endsWith(".arcana") && dir !== Flag.ARCANA_CONFIG_DIR) continue
