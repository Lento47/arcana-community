...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run arcana with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running arcana server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ARCANA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ARCANA_SERVER_USERNAME or 'arcana')",
      })
      .option("dir", {
        type: "string",
