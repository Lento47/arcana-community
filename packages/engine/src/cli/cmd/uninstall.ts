...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall arcana and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      npm: ["npm", "uninstall", "-g", "arcana"],
      pnpm: ["pnpm", "uninstall", "-g", "arcana"],
      bun: ["bun", "remove", "-g", "arcana"],
      yarn: ["yarn", "global", "remove", "arcana"],
      brew: ["brew", "uninstall", "arcana"],
      choco: ["choco", "uninstall", "arcana"],
      scoop: ["scoop", "uninstall", "arcana"],
    }
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "arcana", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
