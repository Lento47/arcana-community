...
import type { TuiThemeCurrent } from "@arcana/plugin/tui"
...
type SharedSyntaxTheme = TuiThemeCurrent & {
  ...
  borderThinking: TuiThemeCurrent["border"]
  surfaceAlt: TuiThemeCurrent["backgroundPanel"]
}
...
export async function resolveRunTheme(renderer: CliRenderer): Promise<RunTheme> {
  try {
    ...
    const shared = await import("@arcana/tui/context/theme")
