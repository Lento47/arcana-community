...
// test/preload.ts pins ARCANA_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.ARCANA_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.ARCANA_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.ARCANA_MODELS_PATH = undefined
  Flag.ARCANA_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.ARCANA_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.ARCANA_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
