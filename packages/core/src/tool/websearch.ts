...
export class ConfigService extends Context.Service<ConfigService, Config>()("@arcana/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ARCANA_WEBSEARCH_PROVIDER === "exa" || process.env.ARCANA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ARCANA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ARCANA_EXPERIMENTAL") || truthy("ARCANA_ENABLE_EXA") || truthy("ARCANA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ARCANA_ENABLE_PARALLEL") || truthy("ARCANA_EXPERIMENTAL_PARALLEL"),
