...
const USER_AGENT = `arcana/${InstallationChannel}/${InstallationVersion}/${Flag.ARCANA_CLIENT}`
...
declare const ARCANA_MODELS_DEV: Record<string, Provider> | undefined
...
export class Service extends Context.Service<Service, Interface>()("@arcana/ModelsDev") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.ARCANA_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.ARCANA_MODELS_PATH ?? filepath).pipe(
      Effect.catch((error) => {
        if (
          Flag.ARCANA_MODELS_PATH === undefined &&
    ...
    const loadSnapshot = Effect.sync(() =>
      typeof ARCANA_MODELS_DEV === "undefined" ? undefined : ARCANA_MODELS_DEV,
    )
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.ARCANA_DISABLE_MODELS_FETCH) return {}
    ...
    if (!Flag.ARCANA_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
